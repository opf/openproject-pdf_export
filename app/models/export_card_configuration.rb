...
class ExportCardConfiguration < ActiveRecord::Base
  class RowsYamlValidator < ActiveModel::Validator
    ...
    def assert_required_keys(hash, valid_keys, required_keys)
      ...
      fail(ArgumentError, "#{I18n.t('validation_error_required_keys_not_present')} #{pending_keys.join(', ')}") unless pending_keys.empty?
    end

    def check_valid_value_type(value, type)
      fail(ArgumentError, "#{I18n.t('validation_error_yaml_is_badly_formed')}") unless value.is_a? type
    end
    ...
    def validate(record)
      ...
      begin
        ...
        groups.each do |_gk, gv|
          ...
          raise_yaml_error unless gv['rows'].is_a?(Hash)
          gv['rows'].each do |_rk, rv|
            assert_required_keys(rv, VALID_ROW_KEYS, REQUIRED_ROW_KEYS)
            raise_yaml_error unless rv['columns'].is_a?(Hash)
  ...
  def self.default
    ExportCardConfiguration.active.find(&:is_default?) || ExportCardConfiguration.active.first
  end
  ...
  def rows_hash
    ...
    fail BadlyFormedExportCardConfigurationError.new(I18n.t('validation_error_yaml_is_badly_formed')) unless config.is_a?(Hash)
    config
  end
